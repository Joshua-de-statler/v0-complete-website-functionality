  ...
  return (
    <div className="space-y-4 sm:space-y-6 h-full flex flex-col">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          <h2 className="text-2xl sm:text-3xl font-bold text-[#EDE7C7]">Conversations</h2>
          <p className="text-sm sm:text-base text-[#EDE7C7]/60 mt-2">View your bot's chat history.</p>
        </div>
        <Button
          variant="outline"
          size="icon"
          ...
          disabled={isLoading}
          className="border-[#2A2A2A] text-[#EDE7C7]/60 hover:text-[#EDE7C7] hover:bg-[#2A2A2A] self-start sm:self-auto bg-transparent"
        >
                ...
                <div className="space-y-1">
                  {filteredConversations.map((conv) => (
                    <button
                      ...
                      className={cn(
                        "w-full p-4 flex items-start gap-3 hover:bg-[#2A2A2A]/50 transition-colors text-left",
            ...
            <>
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <CardTitle className="text-[#EDE7C7] truncate">{selectedConversation.customerName}</CardTitle>
                      <Badge
                        variant="outline"
                        className={cn(
                          "mt-1 text-xs",
                ...
                <ScrollArea className="h-full pr-4">
                  <div className="space-y-4">
                    {selectedConversation.history?.map((message, index) => (
                      <div
                        key={index}
                        className={cn("flex", message.type === "human" ? "justify-start" : "justify-end")}
                      >
                        <div
                          className={cn(
                        ...
                        >
                          <p className="text-sm whitespace-pre-wrap break-words">{message.data.content}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="p-4 border-t border-[#2A2A2A] flex-shrink-0">
                <div className="flex gap-2">
                  ...
                  />
                  <Button
                    className="bg-[#EDE7C7] text-[#0A0A0A] hover:bg-[#EDE7C7]/90 flex-shrink-0"
