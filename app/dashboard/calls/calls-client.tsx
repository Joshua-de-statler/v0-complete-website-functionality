...
import { Search, Phone, Clock, AlertTriangle, PhoneIncoming, Eye, Save, RefreshCw } from "lucide-react"
...
// Helper function to format a call entry
...
export default function CallsClient() {
    ...
    if (companySupabase) {
      ...
      const handleUpdates = (payload: any) => {
        ...
        setCallHistory((currentHistory) =>
          currentHistory
            .map((call) => (call.id === updatedCall.id ? updatedCall : call))
  ...
  const handleSaveNotes = async () => {
    ...
    const updates = { notes: currentNotes }
    try {
                ...
                <div className="flex items-center justify-center h-full min-h-[200px]">
                  <div className="text-center px-6 py-8">
                    ...
                    <p className="text-sm text-[#EDE7C7]/60">
                      Your bot's call logs will appear here once calls are made.
                    </p>
          ...
          <CardContent className="flex-1 overflow-y-auto pr-2">
            {selectedCall ? (
              <div className="space-y-6 text-sm">
                ...
                <div className="space-y-3 border-t border-[#2A2A2A] pt-4">
                  ...
                  {selectedCall.monthly_budget !== null && (
                    <div className="flex justify-between">
                      ...
                      <span className="text-[#EDE7C7]">{formatBudget(selectedCall.monthly_budget)}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="border-t border-[#2A2A2A] pt-4 space-y-2">
                  ...
                  {selectedCall.disqualification_reason && (
                    <div>
                      ...
                      <p className="text-[#EDE7C7] mt-1 text-sm bg-[#0A0A0A] p-3 rounded border border-[#2A2A2A] whitespace-pre-wrap max-h-40 overflow-y-auto">
                        {selectedCall.disqualification_reason}
                      </p>
                    </div>
                  )}
                ...
                <div className="border-t border-[#2A2A2A] pt-4">
                  <Label htmlFor="callNotesEdit" className="text-[#EDE7C7]/80 block mb-2 font-medium">
                    Notes
                  </Label>
                  <Textarea
                    id="callNotesEdit"
                  ...
                  <Button
                    ...
                    disabled={isSavingNotes || !selectedCall}
                    size="sm"
                    className="mt-3 bg-[#EDE7C7]/10 text-[#EDE7C7] hover:bg-[#EDE7C7]/20 border border-[#EDE7C7]/20"
                  >
                    {isSavingNotes ? (
                      "Saving..."
                    ) : (
              ...
              <div className="flex items-center justify-center h-full min-h-[200px]">
                <div className="text-center px-6 py-8">
                  ...
                  <p className="text-sm text-[#EDE7C7]/60">
                    {isLoading && callHistory.length === 0
                      ? "Please wait..."
                      : "Choose a call from the list to view its details."}
                  </p>
                </div>
              </div>
            )}
